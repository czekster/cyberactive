...
if (isset($_POST['id_stix_model']) && !$u->hasPermissionOnModel($_POST['id_stix_model'])) {
   ...
   foreach($_POST as $key => $value) {
      ...
      if ($key == "id_stix_object")
         $id_stix_object = $_POST['id_stix_object'];
      else
      if ($key == "type")
         $type = $_POST['type'];
      else
   ...
   // create STIX object and update 
   $obj = new STIXObject();
   $obj->update($id_stix_type, $name, $u->getIdProfile(), $uuid, $json, $id_stix_object);

   $_SESSION['panel_color'] = "w3-pale-green";
   $_SESSION['message'] = "STIX object edited: '".$name."'.";
   header("Location: edit_new_object.php?id_stix_model=".$id_stix_model);
   exit;
}
?>